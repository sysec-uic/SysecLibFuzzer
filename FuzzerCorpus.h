...
struct InputInfo {
  ...
  size_t NumFocusFunctionsHit = 0;  // How many focus functions were hit
...
public:
  ...
  InputInfo *AddToCorpus(const Unit &U, size_t NumFeatures, bool MayDeleteFile,
    ...
    II.NumFocusFunctionsHit = NumFocusFunctionsHit;
  ...
  void PrintStats() {
    for (size_t i = 0; i < Inputs.size(); i++) {
      ...
      Printf("  [% 3zd %s] sz: % 5zd runs: % 5zd succ: % 5zd focus: %d focusN: %zd\n", i,
             Sha1ToString(II.Sha1).c_str(), II.U.size(),
             II.NumExecutedMutations, II.NumSuccessfulMutations,
             II.HasFocusFunction, II.NumFocusFunctionsHit);
    }
  }
...
private:
  ...
  void UpdateCorpusDistribution(Random &Rand) {
    ...
    if (VanillaSchedule) {
      for (size_t i = 0; i < N; i++) {
        if (!Inputs[i]->NumFeatures) {
          Weights[i] = 0.;
        } else {
          double BaseWeight = static_cast<double>(i + 1);
          // Apply bonus based on how many focus functions were hit
          // Each focus function hit multiplies the weight
          if (Inputs[i]->NumFocusFunctionsHit > 0) {
            // Exponential bonus: each additional function hit doubles the weight
            BaseWeight *= (1 << Inputs[i]->NumFocusFunctionsHit);
          } else if (Inputs[i]->HasFocusFunction) {
            // Backward compatibility: single focus_function
            BaseWeight *= 1000;
          }
