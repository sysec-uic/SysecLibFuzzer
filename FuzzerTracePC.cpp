...
void TracePC::SetFocusFunction(const std::string &FuncName) {
  ...
  Printf("INFO: Looking for focus function '%s'\n", FuncName.c_str());
  Printf("INFO: Scanning %zu modules for function entries...\n", NumModules);
...
// this is the function used to parse and set the functions
// it extended the flags by perfroming addtions to the names
void TracePC::SetFocusFunctions(const std::string &FuncNames) {
  // This function should be called once.
  assert(FocusFunctionsCounterPtrs.empty());
  ...
  size_t Start = 0;
  size_t End = FuncNames.find(',');

  while (End != std::string::npos) {
    std::string FuncName = FuncNames.substr(Start, End - Start);
    // Trim whitespace
    size_t FirstNonSpace = FuncName.find_first_not_of(" \t");
    size_t LastNonSpace = FuncName.find_last_not_of(" \t");
    if (FirstNonSpace != std::string::npos && LastNonSpace != std::string::npos)
      FunctionList.push_back(FuncName.substr(FirstNonSpace, LastNonSpace - FirstNonSpace + 1));
    Start = End + 1;
    End = FuncNames.find(',', Start);
  }

  // Add the last function name
  std::string LastFunc = FuncNames.substr(Start);
  size_t FirstNonSpace = LastFunc.find_first_not_of(" \t");
  size_t LastNonSpace = LastFunc.find_last_not_of(" \t");
  if (FirstNonSpace != std::string::npos && LastNonSpace != std::string::npos)
    FunctionList.push_back(LastFunc.substr(FirstNonSpace, LastNonSpace - FirstNonSpace + 1));

  if (FunctionList.empty())
    return;

  Printf("INFO: Setting up focus on %zu functions\n", FunctionList.size());
  Printf("INFO: Scanning %zu modules for function entries...\n", NumModules);

  // Find each function in the modules
  for (const auto &FuncName : FunctionList) {
    ...
    if (!Found) {
      Printf("WARNING: Failed to find focus function '%s'. Ignoring.\n", FuncName.c_str());
    }
  }

  if (FocusFunctionsCounterPtrs.empty()) {
    Printf("ERROR: None of the specified focus functions were found. "
           "Make sure function names are valid and symbolization is enabled.\n");
    exit(1);
  }

  Printf("INFO: Successfully set up %zu focus functions\n", FocusFunctionsCounterPtrs.size());
}

size_t TracePC::CountObservedFocusFunctions() {
  size_t Count = 0;
  for (auto *CounterPtr : FocusFunctionsCounterPtrs) {
